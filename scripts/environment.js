...
export function drawScene(gl, programInfo, roomElements, viewMatrix, projectionMatrix) {
    ...
    // Disegna le pareti
    roomElements.walls.forEach((wall, index) => {
        const modelMatrix = mat4.create();
        // Posiziona la parete in base all'indice
        switch(index) {
            case 0: mat4.translate(modelMatrix, modelMatrix, [0, 5, -12.5]); break;  // Fronte
            case 1: mat4.translate(modelMatrix, modelMatrix, [0, 5, 12.5]); break;   // Retro
            case 2: mat4.translate(modelMatrix, modelMatrix, [-10, 5, 0]); break;    // Sinistra
            case 3: mat4.translate(modelMatrix, modelMatrix, [10, 5, 0]); break;     // Destra
        }
        drawObject(wall, modelMatrix);
    });
    
    // Disegna pavimento
    const floorMatrix = mat4.create();
    mat4.translate(floorMatrix, floorMatrix, [0, 0, 0]);
    drawObject(roomElements.floor, floorMatrix);
    
    // Disegna soffitto
    const ceilingMatrix = mat4.create();
    mat4.translate(ceilingMatrix, ceilingMatrix, [0, 10, 0]);
    drawObject(roomElements.ceiling, ceilingMatrix);
}
