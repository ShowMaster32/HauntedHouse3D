...
update(deltaTime, playerPosition) {
    ...
    // Verifica confini della stanza
    // Impedisci al giocatore di uscire dalla stanza
    const roomWidth = GAME_CONSTANTS.ROOM.WIDTH;
    const roomDepth = GAME_CONSTANTS.ROOM.DEPTH;
    
    newPosition[0] = Math.max(-roomWidth/2 + 0.5, Math.min(roomWidth/2 - 0.5, newPosition[0]));
    newPosition[2] = Math.max(-roomDepth/2 + 0.5, Math.min(roomDepth/2 - 0.5, newPosition[2]));
